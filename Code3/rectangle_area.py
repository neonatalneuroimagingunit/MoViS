...
win_height = 1200 # screen size
win_width = 1920 # screen size
my_dpi = 96 # dots per inch
...
for hh in np.arange(n_figures):
    figure_name_basis = str(n_points) + '_rectangles_area_'
    ...
    for ii in np.arange(n_points):
        ...
        areas[ii] = total_area/n_points
    
    areas = total_area*areas/areas.sum()
    areas.sort()
    areas = areas[::-1]
    
    side_square = np.sqrt(areas)
    
    side_x = np.zeros(n_points)
    side_y = np.zeros(n_points)
    area_check = np.zeros(n_points)
    for ii in np.arange(n_points):
        ...
        #side_b[ii] = areas[ii]/side_a[ii]
        #area_check[ii] = side_a[ii]*side_b[ii]
        side_x[ii] = 43.67
        side_y[ii] = 43.67
    ...
    grid_step = 1
    #x_max = np.minimum(win_height, win_width)*0.5 # control grid size by monitor size
    x_max = 695
    ...
    allowed_x = np.abs(x_matrix) < (x_max-2*side_x.max())
    allowed_y = np.abs(y_matrix) < (y_max-2*side_y.max())
    allowed = np.logical_and(allowed_x, allowed_y)
...
#    outside = r >= x_max
    ...
    for ii in np.arange(radii.size):
        ...
        allowed_temp = r_temp > (side_x.max()) # you want to play with this (most unfortunate event is two big circles touching if you set to 2 max radius)
    ...
    for aa in np.arange(n_points):
        for bb in np.arange(n_points):
            ...
            egg[aa,bb] = spam[aa,bb] - (side_x[aa]+side_y[aa])/2 - (side_x[bb]+side_y[bb])/2
    ...
    #ax.add_patch(circle)
    # big white rectangle
    rectangle = mpatches.Rectangle((-x_max/2,-x_max/2), x_max, x_max, facecolor=(1,1,1))
    ax.add_patch(rectangle)
    # shapes
    for ii in np.arange(radii.size):
        #shape = mpatches.Circle((centers_coords[ii,0], centers_coords[ii,1]), radii[ii], facecolor=(0,0,0))
        shape = mpatches.Rectangle((centers_coords[ii,0], centers_coords[ii,1]), side_x[ii], side_y[ii], facecolor=(0,0,0))
        ax.add_patch(shape)
