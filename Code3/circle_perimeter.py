# -*- coding: utf-8 -*-
"""
Created on Wed Apr 27 15:44:25 2016
...
total_perimeter = 1000 # total perimeter (around 1000 or less should be optimal)
#min_area = 250 # min area
#max_area = 2500 # max area
#min_area = total_area/10 # min area
#max_area = total_area/2 # max area
min_perimeter = total_perimeter/10
max_perimeter = total_perimeter/2
...
for hh in np.arange(n_figures):
    figure_name_basis = str(n_points) + '_circles_perimeter_'
...
#    check_tot_area = areas.sum()
    ...
    for ii in np.arange(radii.size):
        ...
        allowed_temp = r_temp > (radii.max()*3)
    ...
    for aa in np.arange(n_points):
        for bb in np.arange(n_points):
            ...
            egg[aa,bb] = spam[aa,bb] - radii[aa] - radii[bb]
    ...
    #grid_ax_circ_holes = grid_fig.add_subplot(133)
